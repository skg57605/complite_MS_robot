...
class FullRobotControllerNode(Node):
    ...
    def __init__(self):
        ...
        # --- 역기구학 변수 초기화 ---
        self.l1, self.l2 = 0.2, 0.2  # 슬레이브 로봇의 링크 길이 (미터 단위)
        ...
        self.current_m_joint_3 = 0.0 # 마스터 관절 상태 저장 변수
        self.current_m_joint_4 = 0.0 # 마스터 관절 상태 저장 변수
    ...
    def state_publishing_callback(self):
        ...
        try:
            ...
            for joint_name in joint_names_ordered:
                ...
                # 속도값 읽기 (필요한 조인트만)
                if joint_name in ['s_joint_1', 's_joint_2', 's_end_joint_y', 's_end_joint_z']:
    ...
    def ik_control_callback(self):
        ...
        # --- 제어 블록 2: Z축 제어 (Velocity P-control) ---
        try:
            # 1. 제어 게인(Kp) 및 최대 속도 설정
            kp, max_velocity_rad_s = 3.0, 6.0
            
            # 2. Z축의 물리적 한계(URDF 기준 절대 좌표)
            z_limits = self.joint_limits['s_joint_z'] # 예: { 'min': 0.05, 'max': 0.45 }

            # 3. TF에서 읽어온 목표 Z위치(target_z)를 물리적 한계 내로 제한
            #    target_z는 's_base_link' 기준의 절대 좌표입니다.
            safe_target_z_m = max(z_limits['min'], min(target_z, z_limits['max']))

            # 4. 목표 위치(m)와 현재 위치(m)를 라디안으로 변환
            #    current_s_z_pos_m은 state_publishing_callback에서 이미 보정된 값(m)입니다.
            target_z_rad = self._distance_to_radian(safe_target_z_m)
            current_z_rad = self._distance_to_radian(self.current_s_z_pos_m) 

            # 5. P제어: 목표 위치와 현재 위치의 오차(rad)를 기반으로 속도(rad/s) 계산
    ...
    def solve_ik(self, x, y, current_q1, current_q2):
        ...
        if distance > max_reach:
            self.get_logger().warn(f"Target too far. Clamping.", throttle_duration_sec=1.0)
            x, y, d_sq = x*max_reach/distance, y*max_reach/distance, max_reach**2
        elif distance < min_reach:
            self.get_logger().warn(f"Target too close. Clamping.", throttle_duration_sec=1.0)
            if distance > 0: x, y = x*min_reach/distance, y*min_reach/distance
            else: x, y = min_reach, 0
            d_sq = min_reach**2
